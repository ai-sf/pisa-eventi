"""
Code for the solution of Laplace equation via SOR and wit numba wrapper
"""
import time
import numba
import numpy as np
...
@numba.jit("Tuple((f8[:,:], i8))(i8, f8[:,:], f8[:,:], f8[:,:], f8, f8)", nopython=True, nogil=True)
def solve_lap_sor(N, bound, fixed, rho, w, tau):
    """
    ...
    bound : 2darray
        boundary conditions
    fixed : 2darray
        additional conditions to be respected
    rho : 2darray
        source of field
    w : float,
    ...
    mask = fixed!= 0
    # We need to use loops because numba 
    # doesn't support numpy mask vectorization
    for i in range(1, N):
        for j in range(1, N):
            if mask[i, j]:
                phi[i, j] = fixed[i, j]
...
def pot_block(x, y):
    '''
    Function that checks if (x, y) is a point of the grid
	which belongs to the area of constant potential
    '''
...
if __name__ == "__main__":
    ...
    bound[0, :] = 0.5*(x**2 -x)                      # 0
    bound[1, :] = 1/(1/np.e - np.e)*(np.exp(x)-np.e) # 0
    bound[2, :] = x**4                               #-2
    bound[3, :] = np.cos(np.pi*x/2)                  # 2

    fixed = pot_block(gridx, gridy)
    ...
    # Solution
    start = time.time()
    phi, ic = solve_lap_sor(N, bound, fixed, rho, w, tau)
    end = time.time() - start

    print(f"Number of iterations {ic}")

...
def eulero(N, tf, x0, v0):
    ...
    perche dx/dt = g(x(t)) quindi (x[i+1]-x[i])/dt = g(x[i])
    da cui si isola x[i+1]. Dove x e' però un array contenente
    sia le posizioni sia le velocita'; nel codice pero' per 
    semplicità si e' scelto di dividere le due grandezze
    """
